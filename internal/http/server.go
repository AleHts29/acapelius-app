		...
		api.Group(func(priv chi.Router) {
			...
			priv.Group(func(ready chi.Router) {
				...
				ready.Group(func(admin chi.Router) {
					...
					admin.Get("/reports/seasons", s.handleSeasonsOverview)
					admin.Get("/reports/sellers", s.handleSellersWithSales)
					admin.Get("/reports/sales-timeline", s.handleSalesTimeline)
					admin.Post("/settlements", s.handleCreateSettlement)
					admin.Get("/settlements/{sellerId}/detail", s.handleSellerDetail)
					admin.Post("/settlements/{sellerId}/remind", s.handleRemindSeller)
					admin.Post("/settlements/remind-all", s.handleRemindAll)
				})
			})
		})
